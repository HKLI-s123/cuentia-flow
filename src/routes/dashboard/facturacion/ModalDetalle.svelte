            ...
            <div class="space-y-3 text-sm">
              ...
              <div class="flex justify-between">
                ...
                <span class="font-medium text-gray-900">
                  ...
                  {#if !factura.timbrado && new Date(factura.fechaEmision) > new Date(hoyLocal())}
                    <span class="inline-flex ml-1 px-2 py-0.5 text-xs font-medium rounded-full bg-purple-100 text-purple-700 border border-purple-200">Programada</span>
                  {/if}
                </span>
              </div>
              <div class="flex justify-between">
              ...
              {#if factura.ordenCompra}
                <div class="flex justify-between">
                  ...
                  <span class="font-medium text-gray-900">{factura.ordenCompra}</span>
                </div>
              {/if}
              {#if factura.identificador}
                <div class="flex justify-between">
                  <span class="text-gray-600">Identificador:</span>
                  <span class="font-medium text-gray-900">{factura.identificador}</span>
                </div>
              {/if}
            </div>
          </div>
        ...
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="text-center">
              ...
              <div class="text-2xl font-bold text-green-600">{formatearMoneda(factura.montoTotal ? factura.montoTotal - (factura.saldoPendiente || 0) : 0)}</div>
            </div>
          </div>
        </div>
        ...
        {#if factura.notasCliente || factura.notasInternas}
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {#if factura.notasCliente}
              <div class="bg-white border border-gray-200 rounded-lg p-4">
                <h3 class="text-sm font-semibold text-gray-700 mb-2">Notas para el cliente</h3>
                <p class="text-sm text-gray-700 whitespace-pre-line">{factura.notasCliente}</p>
              </div>
            {/if}
            {#if factura.notasInternas}
              <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div class="flex items-center gap-2 mb-2">
                  <h3 class="text-sm font-semibold text-gray-700">Notas internas</h3>
                  <span class="text-xs bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded">Privado</span>
                </div>
                <p class="text-sm text-gray-700 whitespace-pre-line">{factura.notasInternas}</p>
              </div>
            {/if}
          </div>
        {/if}
