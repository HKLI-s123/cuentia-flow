<script lang="ts">
	...
	// Función para buscar productos SAT desde Facturapi o usando catálogo local
			...
			try {
				...
				if (organizacionId) {
					try {
						...
						console.warn('No se pudo conectar con FacturaAPI, usando catálogo local:', error);
					}
				}

				// Si hay resultados de FacturaAPI, usar esos
				if (resultadosAPI.length > 0) {
					productosSATFiltrados = resultadosAPI;
				} else {
					// Si no, usar el catálogo local de clavesSAT
					productosSATFiltrados = clavesSAT.filter(sat => 
						sat.value.includes(searchTerm) || sat.label.toLowerCase().includes(searchTerm)
					).slice(0, 20);
				}

				mostrarListaProductosSAT = productosSATFiltrados.length > 0;
			} catch (error) {
	...
	function seleccionarProductoSAT(producto: any) {
		// Manejar ambos formatos: FacturaAPI (key/description) y catálogo local (value/label)
		const clave = producto.key || producto.value;
		const descripcion = producto.description || producto.label;
		
		productoServicio = clave;
		busquedaProductoSAT = `${clave} - ${descripcion}`;
		mostrarListaProductosSAT = false;
	}
											...
											{#each productosSATFiltrados as producto}
												...
												>
													...
													<div class="text-xs text-gray-500 mt-1">
														{producto.description || producto.label}
													</div>
												</button>
											{/each}
										</div>
