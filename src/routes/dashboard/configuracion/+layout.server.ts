...
export const load: LayoutServerLoad = async (event) => {
	...
	// Validar en la BD que sea administrador
	try {
		...
		if (roleCheck.rows.length === 0) {
			// Usuario sin organizaciones: permitir acceso a configuración (para contratar plan)
			return {};
		}
		
		if (roleCheck.rows[0].rolname !== 'Administrador') {
